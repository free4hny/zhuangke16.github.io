...
let redColor = d3.scaleLinear().range(['#FFAEC9', '#ED1C24']);
let blueColor = d3.scaleLinear().range(['#99D9EA', '#00A2E8']);
	...
	updateM.enter().append('path')
		...
		.style("fill",function(d){
			...
			if(fillLabel === "i" && !!good){
				...
				return obj? redColor(obj["val"+year]) : '#FFAEC9';
			}else if(fillLabel === "e" && !!good){
				let data = goodData.exctyall;
				let obj = data.find(s=>s['statename'] === usName && s['abbreviatn']==='World');
				return obj? blueColor(obj["val"+year]) : '#FFAEC9';
			}else {
				return '#4292c6';
			}
		});
